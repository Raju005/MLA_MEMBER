...
#End Region
        ...
        If txt_docno.Text <> "" Or txt_docno.Enabled = False Then
            ...
            If CMBLETERNO.Text = "1" Then
                If Cmb_ReminderType.Text = "MEMBER" Then
                    ...
                    ssql = "SELECT FROMDATE,TODATE,ASONDATE,MEMBERTYPECODE,round(BALANCE,0) as balance,MCODE,MNAME,REMINDERDATE,ADDUSER,ADDDATETIME,VOID,VOIDDATETIME,DOCNO,DOCDATE,REMINDERNO,CREDITLIMIT,REFNO,CAST(REFNO_DET AS VARCHAR) AS REFNO_DET,CMONTH,PMONTH,PUSAGE,CONTADD1,CONTADD2,CONTADD3,CONTCITY,CONTCOUNTRY,CONTEMAIL,CONTSTATE,CONTCELL,CONTPIN,CONTPHONE1,CONTPHONE2,TYPE as MEMBERTYPE,SALUT,'MEMBER'as VOIDUSER FROM MM_REMINDERHISTORY_BILL WHERE ISNULL(FROMDATE,'')='" & Format(dtpASNODATE.Value, "yyyy-MM-dd") & "' AND ISNULL(DOCNO,0)=" & txt_docno.Text & " AND ISNULL(VOID,'')<>'Y'   ORDER BY MCODE"
                ...
                ElseIf Cmb_ReminderType.Text = "NRI MEMBER" Then
                    ...
                    ssql = "SELECT FROMDATE,TODATE,ASONDATE,MEMBERTYPECODE,round(P_BALANCE,0) AS BALANCE,MCODE,MNAME,REMINDERDATE,ADDUSER,ADDDATETIME,VOID,VOIDDATETIME,DOCNO,DOCDATE,REMINDERNO,CREDITLIMIT,REFNO,CAST(REFNO_DET AS VARCHAR) AS REFNO_DET,CMONTH,PMONTH,PUSAGE,CONTADD1,CONTADD2,CONTADD3,CONTCITY,CONTCOUNTRY,CONTEMAIL,CONTSTATE,CONTCELL,CONTPIN,CONTPHONE1,CONTPHONE2,TYPE as MEMBERTYPE,SALUT,'NRI MEMBER'as VOIDUSER  FROM MM_REMINDERHISTORY_BILL WHERE ISNULL(FROMDATE,'')='" & Format(dtpASNODATE.Value, "yyyy-MM-dd") & "' AND ISNULL(DOCNO,0)=" & txt_docno.Text & " AND ISNULL(VOID,'')<>'Y'  ORDER BY MCODE"
                    gconnection.GetValues(ssql)
                ElseIf Cmb_ReminderType.Text = "CHAMBER" Then
        ...
        Try
            ...
            If txt_docno.Text <> "" Or txt_docno.Enabled = False Then
                If CMBLETERNO.Text = "1" Then
                    ...
                    End If
                Else
                    ssql = "SELECT REFNO_DET,MCODE,MNAME,BALANCE+P_BALANCE+G_BALANCE AS BALANCE,SALUT,CONTCELL FROM MM_REMINDERHISTORY WHERE ISNULL(FROMDATE,'')='" & Format(dtpASNODATE.Value, "yyyy-MM-dd") & "' AND ISNULL(DOCNO,0)=" & txt_docno.Text & " AND ISNULL(VOID,'')<>'Y' ORDER BY MCODE"
                    gconnection.GetValues(ssql)
                End If
                Dim Viewer As New REPORTVIEWER
    ...
    Private Sub dtpASNODATE_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dtpASNODATE.ValueChanged
        TXT_REFNO.Text = ""
        TXT_REFNO.Text = "Ref: Os/" + Format(dtpASNODATE.Value, "MMM") + "/" + Format(dtpASNODATE.Value, "yyyy")
    End Sub
    ...
    Private Sub Button2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button2.Click
        ...
        Try
            ...
            If CMBLETERNO.Text = "1" Then
                If Cmb_ReminderType.Text = "MEMBER" Then
                    ssql = "insert into reminderhistory(docno,docdate,membertypecode,mcode,mname,fromdate,todate,balance,reminderdate,adduser,adddatetime,reminderno,asondate,creditlimit,REFNO,REFNO_DET,CMONTH,PMONTH,PUSAGE,TYPE)"
                    ssql = ssql & " select '" & docno & "','" & Format(dtp_docdate.Value, "yyyy-MM-dd") & "',MEMBERTYPECODE,mcode,MNAME,'" & Format(dtpASNODATE.Value, "yyyy-MM-dd") & "','" & Format(Dtp_CutOffDate.Value, "yyyy-MM-dd") & "',USAGE,'" & Format(Dtp_rem.Value, "yyyy-MM-dd") & "','" & gUsername & "','" & Format(Now(), "yyyy-MM-dd HH:MM") & "','" & CMBLETERNO.Text & "','" & Format(Dtp_Last.Value, "yyyy-MM-dd") & "'," & Txt_CreditLimit.Text & ",'" & TXT_REFNO.Text & "',REF_ORDER,CMONTH,PMONTH,PUSAGE,'MEMBER' from VIEW_MEM_CREDIT_REMINDER where  isnull(USAGE,0) >= " & Format(Val(Txt_CreditLimit.Text), "0.00") & " AND "
                    ssql = ssql & " isnull(MEMBERTYPECODE,'') in ("
                    'vSql = vSql & " isnull(MT.TypeDesc,'') in ("
                    For I = 0 To CHKCATEGORY.CheckedItems.Count - 1
                        appcode = Split(CHKCATEGORY.CheckedItems(I), ".")
                    ...
                    'ElseIf Cmb_ReminderType.Text = "PARTY" Then
                ElseIf Cmb_ReminderType.Text = "NRI MEMBER" Then
                    ssql = "insert into reminderhistory(docno,docdate,membertypecode,mcode,mname,fromdate,todate,P_balance,reminderdate,adduser,adddatetime,reminderno,asondate,creditlimit,REFNO,REFNO_DET,CMONTH,PMONTH,PUSAGE,TYPE)"
                    ssql = ssql & " select '" & docno & "','" & Format(dtp_docdate.Value, "yyyy-MM-dd") & "',MEMBERTYPECODE,mcode,MNAME,'" & Format(dtpASNODATE.Value, "yyyy-MM-dd") & "','" & Format(Dtp_CutOffDate.Value, "yyyy-MM-dd") & "',P_USAGE,'" & Format(Dtp_rem.Value, "yyyy-MM-dd") & "','" & gUsername & "','" & Format(Now(), "yyyy-MM-dd HH:MM") & "','" & CMBLETERNO.Text & "','" & Format(Dtp_Last.Value, "yyyy-MM-dd") & "'," & Txt_CreditLimit.Text & ",'" & TXT_REFNO.Text & "',REF_ORDER,CMONTH,PMONTH,PUSAGE,'NRI MEMBER' from VIEW_MEM_CREDIT_REMINDER where  isnull(P_USAGE,0) >= " & Format(Val(Txt_CreditLimit.Text), "0.00") & " AND "
                    ssql = ssql & " isnull(MEMBERTYPECODE,'') in ("
                    'vSql = vSql & " isnull(MT.TypeDesc,'') in ("
                    For I = 0 To CHKCATEGORY.CheckedItems.Count - 1
                        appcode = Split(CHKCATEGORY.CheckedItems(I), ".")
